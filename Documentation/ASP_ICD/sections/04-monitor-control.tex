...
\begin{table}[htbp]
    ...
    \begin{tabular}{lp{8cm}}
        ...
        \mib{FEEPOLCUR\_ALL} & All FEE current draws from the most recent poll as a binary-packed array of big-endian signed 16-bit integers in mA, ordered as pol.\ 1, pol.\ 2 for each stand in sequence \\
        \bottomrule
    \end{tabular}
\end{table}
...
The \mib{FEEPOLCUR\_ALL} entry is also new in Version~I. It returns all FEE current draws from the most recent monitoring poll in a single response. The data is packed as an array of big-endian signed 16-bit integers in milliamps, with values interleaved as pol.\ 1, pol.\ 2 for each stand. A value of $-1$ indicates an error or missing data.
\end{newInVersion}
...
The \cmd{INI} command initializes the ASP subsystem (Table~\ref{tab:cmd-ini}). The number of ARX boards is determined from the configuration file.

\begin{calloutBox}{Recent Change}{orange}
Earlier revisions of this ICD specified that \cmd{INI} took the number of ARX boards as an integer data argument. The command no longer accepts any data; the board count is now derived from the \texttt{sub20\_antenna\_mapping} entry in the configuration file.
\end{calloutBox}
